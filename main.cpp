...
hitable *randomScene()
{
			...
			if ((center - vec3(4, 0.2, 0)).length() > 0.9)
			{
				...
				if (chooseMat < 0.7)
				{
					list[i++] = new movingSphere(center, center + vec3(0, rand() / float(RAND_MAX), 0), 0.0, 2.0, 0.2,
						new lambertian(new constTexture(vec3(rand() / float(RAND_MAX)*rand() / float(RAND_MAX), rand() / float(RAND_MAX)*rand() / float(RAND_MAX), rand() / float(RAND_MAX)*rand() / float(RAND_MAX)))));
				}
	...
	list[i++] = new sphere(vec3(-2, 1, -3), 1.0, new lambertian(new constTexture(vec3(0.4, 0.2, 0.6))));
...
//	hitable *world = randomScene();
	...
	ofstream img;
	img.open("C:\\ray tracing image\\twoSphere-perlin.ppm");
